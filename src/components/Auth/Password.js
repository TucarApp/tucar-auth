...
const SimplePasswordChange = () => {
  ...
  const handleChangePassword = async () => {
      try {
          const response = await fetch('https://account-service-1032838122231.us-central1.run.app/api/v1/users/change-password', {
              method: 'POST',
              headers: {
  ...
  const handleConfirmChangePassword = async () => {
      try {
          const response = await fetch('https://account-service-1032838122231.us-central1.run.app/api/v1/users/confirm-change-password', {
              method: 'POST',
              headers: {
                  "Content-Type": "application/json"
              },
              ...
              <div className=" max-w-md p-6 rounded-md text-[#333333]">
                  ...
                  <div className='flex flex-col'>
                      ...
                      <div className="flex gap-2 mt-6">
                          ...
                          {isCodeSent ? (
                              <AuthButton className="flex-1 w-full max-w-[348px] h-[42px] flex justify-center items-center gap-[10px] rounded-[10px] border border-[#E3EDF7] bg-[#E3EDF7] shadow-[inset_-6px_-6px_20px_#FFF,4px_4px_20px_rgba(111,140,176,0.41)] mt-[20px] text-[#5B5D71] text-[15px]" onClick={handleConfirmChangePassword}>
                                  Confirmar cambio
                              </AuthButton>
                          ) : (
                              <AuthButton className="flex-1 w-full max-w-[348px] h-[42px] flex justify-center items-center gap-[10px] rounded-[10px] border border-[#E3EDF7] bg-[#E3EDF7] shadow-[inset_-6px_-6px_20px_#FFF,4px_4px_20px_rgba(111,140,176,0.41)] mt-[20px] text-[#5B5D71] text-[15px]" onClick={handleChangePassword}>
                                  Continuar
                              </AuthButton>
                          )}
                      </div>
